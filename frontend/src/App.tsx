...
function App() {
  ...
  const fetchQueueCount = async () => {
    try {
        ...
        const active = queueData.filter((q: any) => q.status === 'queued' || q.status === 'preparing' || q.status === 'running');
  ...
  const runningJobs = Object.values(jobs).filter(j => j.status === 'running' || j.status === 'preparing' || j.status === 'queued') as Job[];
